...
unsigned int H1(int i, int j, int dimensao) {
    return (i * 1 + j * 2) % (dimensao * dimensao);  // Deve usar dimensao * dimensao
}

unsigned int hashSquare(int i, int j, int colisoes, int dimensao) {
    return (H1(i, j, dimensao) + colisoes * colisoes) % (dimensao * dimensao);
}

int hashPesquisa(Celula* hash, int dimensao, int i, int j) {
    int colisoes = 0;
    unsigned int posicao = hashSquare(i, j, colisoes, dimensao);

    while ((hash[posicao].i != i || hash[posicao].j != j)) {
        colisoes++;
        
        posicao = hashSquare(i, j, colisoes, dimensao);
